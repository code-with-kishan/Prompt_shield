...
def build_document() -> tuple[Document, dict]:
    ...
    _add_para(doc, "Repository: github.com/code-with-kishan/Prompt_Shield_Microsoft_Hackathon", align="center", size=10)
    ...
    _add_para(
        ...
        "in Section 5.6. All claims, limitations, and reproduction scripts are released together with "
        "the paper.",
    )
    ...
    _add_para(doc, "The primary contributions of this v2.0 revision are:")
...
def build_evaluation(doc: Document, datasets: dict) -> None:
    ...
    _mono(doc, "git clone https://github.com/code-with-kishan/Prompt_Shield_Microsoft_Hackathon")
    ...
    _add_para(
        ...
        "test suite comprises 906 tests on the current main branch (as of v3.0) and passes "
        "cleanly across Python 3.10, 3.11, 3.12, and 3.13 in continuous integration.",
    )

    # ----------------------- NEW: 5.6 ACADEMIC BENCHMARKS (v3) ----------
    doc.add_heading("5.6 Independent evaluations against peer-reviewed academic benchmarks (v3)", level=2)
    _add_para(
        doc,
        ...
        "are regex-only (the d022 ML classifier is disabled) for reproducibility on commodity "
        "hardware, matching the configuration used as the baseline column in Table 1.",
    )
    ...
    _add_table(doc, headers, rows)
    _add_para(doc, "Table 2. Detection rate against three peer-reviewed academic benchmarks (regex-only configuration; d022 ML classifier disabled).", italic=True, size=9)
...
def build_conclusions_and_refs(doc: Document) -> None:
    ...
    _add_para(
        ...
        "github.com/code-with-kishan/Prompt_Shield_Microsoft_Hackathon under the Apache 2.0 license. Relevant paths:",
    )
    artifacts = [
